...
def load_pipeline(config: dict):
    ...
    This wrapper supports both versions of model_loader.py:
    - with quantization arguments;
    - without quantization arguments.
    """
    ...
    try:
        return load_text_to_image_pipeline(
            ...
            quantized_components=model_config.get("quantized_components"),
        )
    except TypeError:
        return load_text_to_image_pipeline(
            model_id=model_config["huggingface_id"],
...
def main() -> None:
    ...
    pipeline, actual_device = load_pipeline(config)
